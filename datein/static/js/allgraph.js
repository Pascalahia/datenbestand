...
function get_data5(id) {
    $.get(`/datein/data/${id}/`, function (data) {
        dataset = data.information.information.density_information.information;
        labels = [];
        final_data = [];
        for(let x in dataset) {
            labels.push(x);
        ...
        let ctx = document.getElementById('density_chart').getContext('2d');
        let myChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: labels,
...
get_data5(window.location.href.split('/').pop())